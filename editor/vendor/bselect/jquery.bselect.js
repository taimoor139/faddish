(function ( $ ) {
	...
	Bselect.prototype.build_template = function(){
		...
		template += "<div class='bselect-active "+elipsis+"'>"+this.wrapSelected(this.settings.defaultText,true)+"</div>";
		
			template += "<div class='bselect-content'>"
		
				if(this.settings.search){
					
					template += "<div class='bselect-search'><input type='text' name='s' placeholder='"+this.settings.defaultTextSearch+"' autocomplete='off'/></div>";
				}
				if(this.jsonData){
	...
	Bselect.prototype.wrapSelected = function(text,init){
       text = (init) ? text : "<i class='las la-times bselect-clear'></i>"+text;
	...
	Bselect.prototype.buildOption = function(id){
        ...
        favorite = (this.settings.favorite) ? '<span class="bselect-fav" id="'+this.jsonData[id]+'"></span>' : '';
		return "<li data-id='"+id+"' class='bselect-item "+this.elipsis+disabled+"'>"+this.jsonData[id]+favorite+"</li>";
	}
	/**
	 * Done search typing
	 */
	 ...
	 * On item click
	 */
	Bselect.prototype.bindClick = function(e){
		...
		this.list.children().off('click.bselect.click').on('click.bselect.click', function(e){
          ...
          console.log($(this));
        if($(e.target).hasClass('bselect-fav')){
		return true;
		} 
	...
	Bselect.prototype.bind_events = function(){
		...
		$(this.active).on('mousedown.bselect.open', function(e){
            ...
            else if($(e.target).hasClass('bselect-clear')){
		 $(this).trigger('clear.bselect',false);
			}
