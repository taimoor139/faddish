...
window.Tour = (function(my) {
  ...
  my.prototype.showStep = function(step) {
    stepCounter = this.current;

    var position = elementPosition(step.element, this.container, this.padding, step.frame);
  ...
  var elementPosition = function(element, parentEl, padding, frame) {
    if(frame){
        var el = f(element)[0];
        var left = 300;
    } else {
    ...
    return {
      left: parentEl.scrollLeft + position.left - padding + left,
